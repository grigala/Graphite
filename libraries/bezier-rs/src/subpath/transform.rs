...
impl Subpath {
	pub fn offset(&self, distance: f64) -> Subpath {
		...
		for i in 0..bezier_groups.len() - 1 {
			let last_index = bezier_groups[i].len() - 1;

			let bezier1 = bezier_groups[i][last_index];
			let bezier2 = bezier_groups[i + 1][0];
			...
			bezier_groups[i][last_index] = bezier1.trim(0., bezier1_intersections[0]);
			bezier_groups[i + 1][0] = bezier2.trim(bezier2_intersections[0], 1.);
		}
